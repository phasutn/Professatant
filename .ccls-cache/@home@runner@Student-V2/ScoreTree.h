...
void ST::new_avgscore_tree(float value){
  if(rootPtr == NULL){
    STN* newFirstNode = new STN(value, 0);
    if(newFirstNode){
...
void ST::insert_tree(float value, int year){
  bool inserted = false;
  STN* newNode = new STN(value, year);
  STN* t; //current
  if(newNode){
    if(size==0){
      rootPtr = newNode;
...
void ST::print_tree(STN* treePtr, int x, int year, float AvgScore){
  if(treePtr){
    ...
    if (treePtr->get_year() == year){
        ...
        cout<<setw(x*5-5)<<treePtr->get_score()<<endl;
